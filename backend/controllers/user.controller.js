...
const { processCodeforcesData } = require('../utils/processData');
...
exports.getLeetcodeData = async (req, res) => {
    var handle = req.params.handle; // Replace with the desired Codeforces username
    // console.log(handle)
    if (handle) {
        try {
...
exports.getCodeforcesData = async (req, res) => {
    ...
    if (handle) {
        try {
            ...
            const codeforcesData = await processCodeforcesData(response.data.result);
